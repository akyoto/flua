...
inline BPUTF8String* flua_IPv4ToString(Int32 intIP) {
...
inline BPUTF8String* flua_IPv6ToString(uint8_t* ptr) {
...
public:
	inline BPUTF8String* toUTF8String() {
		if(af == AF_INET) {
			return flua_IPv4ToString(castIPv4(addr)->sin_addr.s_addr);
		} else {
			return flua_IPv6ToString(castIPv6(addr)->sin6_addr.s6_addr);
		}
	}
...
// flua_getHostIP
inline BPIPInfo* flua_getHostIP(BPUTF8String* hostName) {
