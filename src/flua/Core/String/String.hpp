...
template <typename T>
inline size_t flua_strlen(T ptr) {
	return strlen(ptr);
}

// TODO: Optimize
inline size_t flua_strlenUtf8Xchars(char* _s, size_t _limit) {
	char *start = _s;
	char b;
	size_t count = 0;
	_limit++;
...
inline size_t flua_strlenUtf8(char* _s) {
	char b;
	size_t count = 0;
...
// flua_insertInt
template <typename T>
inline size_t flua_insertInt(char* ptr, const T num, const size_t index) {
